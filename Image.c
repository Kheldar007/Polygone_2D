...
{	
	...
	int i = xA , j = yA ;
	glColor3f (1 , 1 , 1) ; // Couleur blanche.	
	if (deltaX > 0) // Moitie droite.
	{
		if (deltaY > 0) // Quart en haut a droite.
		{
		...
		else if (deltaY < 0) // Quart en bas a droite.
		{
	...
	else if (deltaX < 0) // Moitie gauche.
	{
		if (deltaY > 0) // Quart en haut a gauche.
		{
		...
		else if (deltaY < 0) // Quart en bas a gauche.
		{
	...
	else if (deltaY == 0) // A l'horizontale.
	{
		...
		else if (deltaX < 0) // Vers la gauche.
		{
			while (i > xB)
			{
				I_plot (img , i , j) ;
